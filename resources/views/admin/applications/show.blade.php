                                ...
                                @foreach($application->answers as $answer)
                                    <div class="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900/50 dark:to-gray-800/50 rounded-xl p-5 last:mb-0">
                                        <div class="flex items-start gap-3 mb-3">
                                            ...
                                            <p class="text-sm font-semibold text-gray-900 dark:text-white flex-1">{{ $answer->question->question_text }}</p>
                                        </div>
                                        @if($answer->answer_type === 'text')
                                            <p class="text-sm text-gray-700 dark:text-gray-300 ml-10 leading-relaxed">{{ $answer->answer_text }}</p>
                                        @elseif($answer->answer_type === 'video' && $answer->video_path)
                                            <div class="ml-10">
                                                <video controls class="w-full max-w-md rounded-xl shadow-lg">
                                                    <source src="{{ asset('storage/' . $answer->video_path) }}" type="video/mp4">
                                                    Your browser does not support the video tag.
                                                </video>
                                            </div>
                                        @elseif($answer->answer_type === 'file' && $answer->file_path)
                                            <div class="ml-10">
                                                <a href="{{ asset('storage/' . $answer->file_path) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-all">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                                    </svg>
                                                    Download File
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm hover:shadow-md transition-shadow sticky top-24">
                        ...
                        <div class="space-y-3">
                            ...
                            <x-black-button variant="outline" class="w-full justify-center">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                </svg>
                                Send Test
