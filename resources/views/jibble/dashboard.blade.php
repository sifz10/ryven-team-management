<x-app-layout>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-2">{{ $stats['total_time_entries'] ?? 0 }}</p>
                        </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-2">{{ $stats['pending_leave'] ?? 0 }}</p>
                        </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-2">{{ $stats['approved_leave'] ?? 0 }}</p>
                        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('jibble.sync-history') }}" class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-medium">
                            View All →
                        </a>
                    </div>
                </div>
                        ...
                        <div class="p-6 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex-1">
                                        <p class="font-medium text-gray-900 dark:text-white capitalize">{{ str_replace('_', ' ', $sync->sync_type) }}</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">
                                            Synced: {{ $sync->records_synced }} | Failed: {{ $sync->records_failed }}
                                        </p>
                                        @if($sync->error_message)
                                            <p class="text-sm text-red-600 dark:text-red-400 mt-1">{{ $sync->error_message }}</p>
                                        @endif
                                    </div>
                                </div>
                                <div class="text-right">
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">{{ $sync->completed_at?->diffForHumans() ?? $sync->created_at->diffForHumans() }}</p>
                                </div>
                            </div>
                        </div>
                    @empty
    ...
    <script>
        function testConnection() {
            showLoading();
            fetch('{{ route("jibble.test-connection") }}', {
                method: 'POST',
                headers: {
        ...
        function syncEmployees() {
            showLoading();
            fetch('{{ route("jibble.sync-employees") }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
        ...
        function syncTimeEntries() {
            showLoading();
            fetch('{{ route("jibble.sync-time-entries") }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
        ...
        function syncLeaveRequests() {
            showLoading();
            fetch('{{ route("jibble.sync-leave-requests") }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
