...
<body>
    ...
    <div class="content">
        <h2 style="margin-top: 0; color: #1f2937;">Hello {{ $invoice->client_name }},</h2>
        ...
        <div class="invoice-details">
            ...
            <div class="detail-row">
                ...
                <span class="detail-value">{{ $invoice->invoice_date->format('F d, Y') }}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Due Date:</span>
                <span class="detail-value">{{ $invoice->due_date->format('F d, Y') }}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Status:</span>
                <span class="detail-value" style="text-transform: uppercase; font-weight: 600;">{{ $invoice->status }}</span>
            </div>
        </div>

        <div class="total-amount">
            <div class="label">Total Amount</div>
            <div class="amount">{{ $invoice->currency }} {{ number_format($invoice->total, 2) }}</div>
        </div>
        ...
        @if($invoice->terms)
            <div style="margin: 20px 0; padding: 15px; background-color: #fffbeb; border-radius: 8px; border: 1px solid #fbbf24;">
                ...
                <p style="margin: 0; color: #78350f; font-size: 12px; white-space: pre-line;">{{ $invoice->terms }}</p>
            </div>
        @endif
    </div>
