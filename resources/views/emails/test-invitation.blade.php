...
<body>
    <div class="email-container">
        ...
        <div class="header">
            ...
            <p>Test Assignment for {{ $application->jobPost->title }}</p>
        </div>

        <!-- Content -->
        <div class="content">
            <p class="greeting">Hi {{ $application->first_name }},</p>
            ...
            <div style="text-align: center;">
                <span class="deadline-badge">
                    ⏰ Deadline: {{ $test->deadline->format('F j, Y \a\t g:i A') }}
                </span>
            </div>
            ...
            <div style="background-color: #f0fdf4; border-left: 4px solid #10b981; padding: 20px; border-radius: 8px; margin: 25px 0;">
                ...
                <p style="color: #dc2626; margin: 0; font-weight: 600; font-size: 14px;">
                    ⚠️ This link will expire on {{ $test->deadline->format('F j, Y \a\t g:i A') }}
                </p>
            </div>

            <!-- Submit Button -->
            <div class="button-container">
                <a href="{{ route('test.submission.show', $test->submission_token) }}" class="btn">
                    Access Submission Portal
                </a>
            </div>

            <p style="color: #6b7280; font-size: 14px; text-align: center; margin-top: 15px;">
                Submission Link: <a href="{{ route('test.submission.show', $test->submission_token) }}" style="color: #2563eb;">{{ route('test.submission.show', $test->submission_token) }}</a>
            </p>
        </div>

        <!-- Footer -->
        <div class="footer">
            <p><strong>{{ config('app.name') }} - Recruitment Team</strong></p>
