...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f3f4f6;">
    <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
        ...
        <div style="background: linear-gradient(135deg, #1f2937 0%, #111827 100%); padding: 40px 30px; text-align: center;">
            ...
            <p style="margin: 10px 0 0 0; color: #d1d5db; font-size: 16px;">{{ $project->name }}</p>
        </div>
        ...
        <div style="padding: 40px 30px;">
            ...
            <p style="margin: 0 0 20px 0; color: #374151; font-size: 16px; line-height: 1.6;">
                Hello <strong>{{ $uatUser->name }}</strong>,
            </p>

            <p style="margin: 0 0 25px 0; color: #374151; font-size: 16px; line-height: 1.6;">
                <strong>{{ $invitedBy->name }}</strong> has invited you to participate in the User Acceptance Testing (UAT) for <strong>{{ $project->name }}</strong>.
            </p>

            <!-- Role Badge -->
            <div style="margin: 0 0 30px 0; padding: 15px; background-color: {{ $uatUser->role === 'internal' ? '#dbeafe' : '#f3f4f6' }}; border-left: 4px solid {{ $uatUser->role === 'internal' ? '#3b82f6' : '#6b7280' }}; border-radius: 6px;">
                <p style="margin: 0; color: #1f2937; font-size: 14px; font-weight: 600;">
                    Your Role: 
                    <span style="color: {{ $uatUser->role === 'internal' ? '#1e40af' : '#4b5563' }};">
            ...
            <div style="margin: 30px 0 0 0; padding: 25px; background-color: #fef3c7; border-radius: 8px; border: 1px solid #fbbf24;">
                ...
                <ol style="margin: 0; padding-left: 20px; color: #78350f; font-size: 14px; line-height: 1.8;">
                    <li style="margin-bottom: 10px;">
                        ...
                        <code style="background-color: #ffffff; padding: 4px 8px; border-radius: 4px; font-size: 12px; word-break: break-all;">{{ $project->public_url }}</code>
                    </li>
                    <li style="margin-bottom: 10px;">
                        <strong>Enter your email address</strong> ({{ $uatUser->email }}) to authenticate
                    </li>
                    <li style="margin-bottom: 10px;">
            ...
            @if($project->deadline)
                ...
                <div style="margin: 25px 0 0 0; padding: 15px; background-color: #fef2f2; border-radius: 6px; border: 1px solid #fecaca;">
                    <p style="margin: 0; color: #991b1b; font-size: 14px;">
                        <strong>⏰ Deadline:</strong> {{ $project->deadline->format('F d, Y \a\t g:i A') }}
                    </p>
                </div>
            @endif
            ...
            <div style="margin: 30px 0 0 0; padding-top: 20px; border-top: 1px solid #e5e7eb;">
                <p style="margin: 0; color: #6b7280; font-size: 14px; line-height: 1.6;">
                    If you have any questions or need assistance, please contact <strong>{{ $invitedBy->name }}</strong> at <a href="mailto:{{ $invitedBy->email }}" style="color: #3b82f6; text-decoration: none;">{{ $invitedBy->email }}</a> or our support team at <a href="mailto:support@ryven.co" style="color: #3b82f6; text-decoration: none;">support@ryven.co</a>
                </p>
            </div>
        </div>
