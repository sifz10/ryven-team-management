...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900" x-data="{
    ...
    loadFiles() {
        ...
        fetch(`/client/projects/{{ $project->id }}/tasks/{{ $task->id }}/files`, {
            headers: {
    ...
    loadComments() {
        const csrfToken = document.querySelector('meta[name=csrf-token]').getAttribute('content');
        fetch(`/client/projects/{{ $project->id }}/tasks/{{ $task->id }}/comments`, {
            headers: {
                'X-CSRF-TOKEN': csrfToken
            }
        })
    ...
    loadReminders() {
        const csrfToken = document.querySelector('meta[name=csrf-token]').getAttribute('content');
        fetch(`/client/projects/{{ $project->id }}/tasks/{{ $task->id }}/reminders`, {
            headers: {
                'X-CSRF-TOKEN': csrfToken
            }
        })
    ...
    addReply(comment) {
        ...
        const csrfToken = document.querySelector('meta[name=csrf-token]').getAttribute('content');
        const url = '/client/projects/{{ $project->id }}/tasks/{{ $task->id }}/comments/' + comment.id + '/replies';
    ...
    toggleReaction(commentId, reactionType) {
        const csrfToken = document.querySelector('meta[name=csrf-token]').getAttribute('content');
        const url = '/client/projects/{{ $project->id }}/tasks/{{ $task->id }}/comments/' + commentId + '/reactions';
    ...
    handleReplyInput(event, comment) {
        ...
        if (lastAtIndex !== -1) {
            ...
            if (!textAfterAt.includes(' ')) {
                ...
                const csrfToken = document.querySelector('meta[name=csrf-token]').getAttribute('content');
                fetch('/client/projects/{{ $project->id }}/employees/mention?search=' + mentionSearch, {
    ...
    searchMentionEmployees(search) {
        const csrfToken = document.querySelector('meta[name=csrf-token]').getAttribute('content');
        fetch('/client/projects/{{ $project->id }}/employees/mention?search=' + search, {
            headers: { 'X-CSRF-TOKEN': csrfToken }
        })
        .then(response => response.json())
        .then(data => {
    ...
    <div class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-3 sm:px-6 lg:px-8 py-3 sm:py-4">
        <a href="{{ route('client.projects.show', $project) }}?tabs=tasks" class="inline-flex items-center text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors">
        ...
        <div class="lg:col-span-2 space-y-4 sm:space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="bg-black dark:bg-white p-4 sm:p-6">
                    <h1 class="text-lg sm:text-xl lg:text-2xl font-bold text-white dark:text-black mb-2 sm:mb-3 break-words">{{ $task->title }}</h1>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        ...
                        <div class="flex items-center gap-2 sm:gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="font-medium text-sm sm:text-base text-black dark:text-white truncate">{{ $task->assignee->name }}</div>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="flex items-center gap-2 sm:gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="font-medium text-sm sm:text-base text-black dark:text-white">{{ \Carbon\Carbon::parse($task->due_date)->format('M d, Y') }}</div>
                            </div>
                        </div>
                        @endif
                    </div>
            ...
            <div x-data="{
                ...
                async toggleChecklist(checklistId) {
                    ...
                    try {
                        const response = await fetch(`/client/tasks/{{ $task->id }}/checklist/${checklistId}/toggle`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
            ...
            <div x-data="{
                ...
                async loadFiles() {
                    try {
                        const response = await fetch('/client/projects/{{ $project->id }}/tasks/{{ $task->id }}/files');
                ...
                async confirmDeleteFile() {
                    ...
                    try {
                        const deleteUrl = '/client/projects/{{ $project->id }}/tasks/{{ $task->id }}/files/' + this.deletingFileId;
                    ...
                    <div class="space-y-2 sm:space-y-3">
                        <div class="flex items-start gap-2 sm:gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-600 dark:text-gray-300">{{ \Carbon\Carbon::parse($task->created_at)->diffForHumans() }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-2 sm:gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-600 dark:text-gray-300">{{ \Carbon\Carbon::parse($task->updated_at)->diffForHumans() }}</p>
                            </div>
                        </div>
                        @endif
                    </div>
