<div x-data="{
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <span class="text-sm text-gray-600 dark:text-gray-400">{{ $project->files->count() }} files</span>
        </div>
    ...
    @if($project->files->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            @foreach($project->files as $file)
                <div x-show="selectedCategory === 'all' || selectedCategory === '{{ $file->category }}'" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 hover:shadow-lg transition-shadow">
                    <!-- File Icon/Preview -->
                    <div class="aspect-square rounded-lg bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-900 flex items-center justify-center mb-4 overflow-hidden">
                        ...
                        @if(in_array(strtolower($extension), $imageExtensions))
                            <img src="{{ Storage::url($file->file_path) }}" alt="{{ $file->name }}" class="w-full h-full object-cover">
                        @else
                            <div class="text-center">
                                ...
                                @endif
                                <p class="text-sm font-bold text-gray-600 dark:text-gray-400 uppercase mt-2">{{ $extension }}</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <!-- File Info -->
                    <div class="space-y-2">
                        <h4 class="font-semibold text-sm text-gray-900 dark:text-white truncate" title="{{ $file->name }}">{{ $file->name }}</h4>

                        <div class="flex items-center justify-between text-xs">
                            <span class="inline-flex items-center px-2 py-1 rounded-full font-semibold
                                @if($file->category === 'document') bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400
                                @elseif($file->category === 'design') bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400
                                @elseif($file->category === 'code') bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400
                                @elseif($file->category === 'image') bg-pink-100 text-pink-800 dark:bg-pink-900/30 dark:text-pink-400
                                @else bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 @endif">
                                {{ ucfirst($file->category) }}
                            </span>
                            @if($file->size)
                                <span class="text-gray-600 dark:text-gray-400">{{ number_format($file->size / 1024, 2) }} KB</span>
                            @endif
                        </div>
                        ...
                        <div class="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                            ...
                            <span>{{ $file->uploader->first_name ?? 'Unknown' }}</span>
                        </div>
                        ...
                        @if($file->assignee)
                            <div class="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                                ...
                                <span>Assigned to {{ $file->assignee->first_name }} {{ $file->assignee->last_name }}</span>
                            </div>
                        @endif

                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ $file->created_at->diffForHumans() }}</p>
                    </div>

                    <!-- Actions -->
                    <div class="flex items-center gap-2 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <a href="{{ Storage::url($file->file_path) }}" download class="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg text-sm font-semibold hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path>
                            </svg>
                            Download
                        </a>
                        <form action="{{ route('projects.files.destroy', [$project, $file]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this file?')">
                            @csrf
                            @method('DELETE')
