    ...
    <div class="py-8">
        <div x-data="{ tab: getInitialTab() }" class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                                ...
                                <li class="relative pl-16 group">
                                    ...
                                    <div class="bg-white dark:bg-gray-800/60 backdrop-blur-sm border {{ $borderColor }} rounded-xl p-4 shadow-sm hover:shadow-md dark:hover:shadow-xl transition-all duration-200">
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <div class="flex items-center gap-2">
                                                <button type="button" class="inline-flex items-center gap-1 px-3 py-1.5 bg-gray-800 dark:bg-black text-white rounded-full hover:bg-gray-700 dark:hover:bg-gray-900 transition text-xs font-medium shadow-sm" x-data @click="$dispatch('open-modal', 'edit-payment-{{ $payment->id }}')">
                                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                                    </svg>
                                                    Edit
                                    ...
                                    @endif
                                        ...
                                        @if($payment->notes->count() > 0)
                                            <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                                                <div class="flex items-center gap-2 mb-3">
                                                    ...
                                                    <span class="text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Notes ({{ $payment->notes->count() }})</span>
                                                </div>
                                                <div class="space-y-2">
                                                        ...
                                                        <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                                                            <div class="flex items-start justify-between gap-2 mb-2">
                                                                <div class="flex items-center gap-2">
                                                                    ...
                                                                    <div class="text-xs">
                                                                        <span class="font-medium text-gray-900 dark:text-white">{{ $note->user->name ?? 'Unknown' }}</span>
                                                                        <span class="text-gray-500 dark:text-gray-400"> • {{ $note->created_at->diffForHumans() }}</span>
                                                                    </div>
                                                                </div>
                                                                <form method="POST" action="{{ route('employees.payments.notes.destroy', [$employee, $payment, $note]) }}" class="inline">
                                                                    @csrf
                                                                    @method('DELETE')
                                        ...
                                        <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700" x-data="{ showNoteForm: false }">
                                            ...
                                            <form x-show="showNoteForm" x-transition method="POST" action="{{ route('employees.payments.notes.store', [$employee, $payment]) }}" class="mt-3 space-y-3">
                                                @csrf
                                        ...
                                        <div class="p-6">
                                            ...
                                            <form method="POST" action="{{ route('employees.payments.update', [$employee, $payment]) }}?tab=timeline" class="space-y-4" x-data="{ saving: false }" @submit="saving = true">
                                                ...
                                                <div>
                                                    ...
                                                    <x-text-input id="paid_at_{{ $payment->id }}" type="date" name="paid_at" class="mt-1 block w-full" value="{{ \Carbon\Carbon::parse($payment->paid_at)->format('Y-m-d') }}" required />
                                                </div>
                                                <div>
            ...
            <div x-cloak x-show="tab==='github'" x-transition.opacity>
                @include('employees.partials.github-activity', ['employee' => $employee])
            </div>
