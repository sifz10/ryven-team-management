<x-app-layout>
            ...
            <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 py-4">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-2 sm:gap-3" x-data="{ isSyncing: false }">
                            ...
                            <select onchange="window.location.href='?account='+this.value" 
                                ...
                                @foreach($accounts as $account)
                                    <option value="{{ $account->id }}" {{ $currentAccount?->id == $account->id ? 'selected' : '' }}>
                                        {{ $account->name }} ({{ $account->unread_messages->count() }})
                                    </option>
                                @endforeach
                            </select>
                        @endif

                        <a href="{{ route('email.inbox.compose') }}" class="inline-flex items-center px-4 sm:px-6 py-2 bg-black dark:bg-white text-white dark:text-black rounded-full font-semibold text-sm hover:bg-gray-800 dark:hover:bg-gray-100 transition">
                        ...
                        <a href="{{ route('email.accounts.index') }}" class="inline-flex items-center px-4 sm:px-6 py-2 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white rounded-full font-semibold text-sm hover:bg-gray-200 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600 transition">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <form method="GET" class="mt-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search emails..."
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                ...
                <div class="w-full lg:w-64 flex-shrink-0" x-data="{ mobileMenuOpen: false }">
                    ...
                    <div :class="{ 'hidden': !mobileMenuOpen, 'block': mobileMenuOpen }" class="lg:block bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl sm:rounded-3xl p-4 lg:sticky lg:top-24">
                        <nav class="space-y-1">
                            <a href="{{ route('email.inbox.index', ['account' => $currentAccount?->id, 'folder' => 'INBOX']) }}" 
                                class="flex items-center px-4 py-3 rounded-2xl {{ $folder == 'INBOX' ? 'bg-black dark:bg-white text-white dark:text-black' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }} transition">
                            ...
                            <a href="{{ route('email.inbox.index', ['account' => $currentAccount?->id, 'folder' => 'Sent']) }}" 
                                class="flex items-center px-4 py-3 rounded-2xl {{ $folder == 'Sent' ? 'bg-black dark:bg-white text-white dark:text-black' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }} transition">
                                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="{{ route('email.inbox.index', ['account' => $currentAccount?->id, 'folder' => 'Drafts']) }}" 
                                class="flex items-center px-4 py-3 rounded-2xl {{ $folder == 'Drafts' ? 'bg-black dark:bg-white text-white dark:text-black' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }} transition">
                                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="{{ route('email.inbox.index', ['account' => $currentAccount?->id, 'folder' => 'Trash']) }}" 
                                class="flex items-center px-4 py-3 rounded-2xl {{ $folder == 'Trash' ? 'bg-black dark:bg-white text-white dark:text-black' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }} transition">
                                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="{{ route('email.inbox.index', ['account' => $currentAccount?->id, 'folder' => $folder, 'filter' => 'unread']) }}" 
                                class="flex items-center px-4 py-3 rounded-2xl {{ $filter == 'unread' ? 'bg-black dark:bg-white text-white dark:text-black' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }} transition">
                                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="{{ route('email.inbox.index', ['account' => $currentAccount?->id, 'folder' => $folder, 'filter' => 'starred']) }}" 
                                class="flex items-center px-4 py-3 rounded-2xl {{ $filter == 'starred' ? 'bg-black dark:bg-white text-white dark:text-black' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }} transition">
                                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @else
                        ...
                        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-3xl overflow-hidden">
                            ...
                            <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center gap-4">
                                <input type="checkbox" 
                                       x-model="selectAll"
                                       @change="selectAll ? selectedEmails = [{{ $messages->pluck('id')->join(',') }}] : selectedEmails = []"
                            ...
                            @foreach($messages as $message)
                                <div class="group flex items-center p-4 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition {{ !$message->is_read ? 'bg-blue-50 dark:bg-gray-750' : '' }}">
                                    ...
                                    <!-- Star -->
                                    <form action="{{ route('email.inbox.star', $message) }}" method="POST" class="ml-3 flex-shrink-0" onclick="event.preventDefault(); event.stopPropagation(); this.submit();">
                                        @csrf
                                        <button type="submit" class="text-gray-400 hover:text-yellow-500 transition">
                                            <svg class="w-5 h-5 {{ $message->is_starred ? 'fill-yellow-500 text-yellow-500' : '' }}" fill="{{ $message->is_starred ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path>
                                            </svg>
                                        </button>
                                    </form>
                                    ...
                                    <a href="{{ route('email.inbox.show', $message) }}" class="flex flex-col sm:flex-row sm:items-center flex-1 min-w-0 ml-4 gap-2 sm:gap-0">
                                        <!-- From -->
                                        <div class="sm:w-48 flex-shrink-0">
                                            <p class="font-semibold text-sm sm:text-base {{ !$message->is_read ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400' }} truncate">
                                        ...
                                        <div class="flex-1 sm:mx-6 min-w-0">
                                            <p class="font-semibold text-sm sm:text-base {{ !$message->is_read ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400' }} truncate">
                                                {{ Str::limit($message->subject ?: '(No Subject)', 60) }}
                                            </p>
                                            <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-500 truncate">
                                                {{ Str::limit(strip_tags($message->body_text), 80) }}
                                            </p>
                                        </div>
                                    ...
                                    <div class="flex gap-2 ml-3 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" onclick="event.preventDefault(); event.stopPropagation();">
                                        @if($folder === 'Trash')
                                            <form action="{{ route('email.inbox.restore', $message) }}" method="POST" class="inline">
                                                @csrf
                                            ...
                                            <form action="{{ route('email.inbox.destroy', $message) }}" method="POST" class="inline" onsubmit="return confirm('Permanently delete this email?')">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @else
                                            <form action="{{ route('email.inbox.trash', $message) }}" method="POST" class="inline">
                                                @csrf
                        ...
                        <div class="mt-6 flex items-center justify-between">
                            <div class="flex items-center gap-4">
                                <p class="text-sm text-gray-600 dark:text-gray-400">
                                    Showing <span class="font-semibold">{{ $messages->firstItem() ?? 0 }}</span> 
                                    to <span class="font-semibold">{{ $messages->lastItem() ?? 0 }}</span> 
                                    of <span class="font-semibold">{{ $messages->total() }}</span> emails
                                </p>
                                
                                <!-- Per Page Selector -->
                                <select onchange="window.location.href='?account={{ $currentAccount?->id }}&folder={{ $folder }}&filter={{ $filter }}&search={{ request('search') }}&per_page='+this.value" 
    ...
    <script>
        ...
        function triggerSync() {
            @if($currentAccount)
                ...
                fetch('{{ route("email.accounts.sync", ["account" => $currentAccount->id ?? 0]) }}', {
                    method: 'POST',
                    headers: {
    ...
    <script>
        ...
        window.bulkAction = function(action, selectedIds) {
            ...
            form.method = 'POST';
            form.action = '{{ route("email.inbox.bulk-action") }}';
