                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                            <div class="flex flex-col">
                                                <span class="font-medium">{{ $log->event_at->format('M d, Y') }}</span>
                                                <span class="text-xs text-gray-500 dark:text-gray-400">{{ $log->event_at->format('h:i A') }}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            @if($log->action)
                                                <span class="block text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $log->action }}</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 text-sm">
                                            <div class="flex flex-col max-w-xs">
                                                <a href="{{ $log->repository_url }}" target="_blank" class="font-medium text-gray-900 dark:text-gray-100 hover:text-blue-600 dark:hover:text-blue-400 truncate">
                                                    {{ $log->repository_name }}
                                                </a>
                                                @if($log->branch)
                                            ...
                                            <div class="max-w-md">
                                                @if($log->event_type === 'push')
                                                    <div class="flex items-start gap-2">
                                                        ...
                                                        <div>
                                                            <div class="font-medium">{{ $log->commits_count }} {{ Str::plural('commit', $log->commits_count) }}</div>
                                                            @if($log->commit_message)
                                                                <div class="text-xs text-gray-600 dark:text-gray-400 mt-1 truncate">{{ Str::limit($log->commit_message, 60) }}</div>
                                                            @endif
                                                        </div>
                                                    </div>
                                                @elseif($log->event_type === 'pull_request' || $log->event_type === 'issues')
                                                    <div>
                                                        <div class="font-medium">{{ $log->pr_title }}</div>
                                                        @if($log->pr_number)
                                                            <span class="text-xs text-gray-500 dark:text-gray-400">#{{ $log->pr_number }}</span>
                                                        @endif
                                                    </div>
                                                @else
                                                    <div class="text-gray-600 dark:text-gray-400">{{ Str::limit($log->commit_message, 60) }}</div>
                                                @endif
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <div class="flex items-center gap-2">
                                                @if($log->event_type === 'pull_request')
                                                    <button @click="$dispatch('open-pr-modal', {{ $log->id }})" class="inline-flex items-center gap-1 px-3 py-1 bg-black text-white rounded-full hover:bg-gray-800 transition-all text-xs font-medium">
                                                ...
                                                @elseif($log->commit_url || $log->pr_url)
                                                    <a href="{{ $log->commit_url ?? $log->pr_url }}" target="_blank" class="inline-flex items-center gap-1 px-3 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-all text-xs font-medium">
                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                @endif
                                                @if($log->employee)
                                                    <a href="{{ route('employees.show', $log->employee) }}?tab=github" class="inline-flex items-center gap-1 px-3 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-all text-xs font-medium">
                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                                        </svg>
                                                        Profile
