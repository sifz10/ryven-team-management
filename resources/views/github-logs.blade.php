                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <div class="flex items-center gap-3">
                                                @if($log->author_avatar_url)
                                                    <img src="{{ $log->author_avatar_url }}" alt="{{ $log->author_username }}" class="w-8 h-8 rounded-full">
                                                @else
                                                    <div class="w-8 h-8 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center">
                                                        <span class="text-xs font-medium text-gray-600 dark:text-gray-300">
                                                            {{ strtoupper(substr($log->author_username, 0, 2)) }}
                                                        </span>
                                                    </div>
                                                @endif
                                                <div>
                                                    <div class="font-medium text-gray-900 dark:text-gray-100">
                                                        {{ $log->employee->first_name ?? 'Unknown' }} {{ $log->employee->last_name ?? '' }}
                                                    </div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <div class="flex items-center gap-2">
                                                ...
                                                @endif
                                                @if($log->employee)
                                                    <a href="{{ route('employees.show', $log->employee) }}?tab=github" class="inline-flex items-center gap-1 px-3 py-1 bg-black text-white rounded-full hover:bg-gray-800 transition-all text-xs font-medium">
                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                                        </svg>
                                                        Profile
