    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-4 mb-4">
                <a href="{{ route('tickets.index') }}" class="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-all">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                    <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-4">{{ $ticket->title }}</h2>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <a href="{{ route('projects.show', $ticket->project) }}" class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-600 transition-all group">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="font-medium text-gray-900 dark:text-white truncate">{{ $ticket->project->name }}</p>
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center flex-shrink-0">
                                    <span class="text-sm font-bold text-gray-600 dark:text-gray-300">
                                        {{ substr($ticket->reportedBy->first_name, 0, 1) }}{{ substr($ticket->reportedBy->last_name, 0, 1) }}
                                    </span>
                                </div>
                                <div>
                                    <p class="font-medium text-gray-900 dark:text-white text-sm">
                                        {{ $ticket->reportedBy->first_name }} {{ $ticket->reportedBy->last_name }}
                                    </p>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 bg-black dark:bg-white rounded-full flex items-center justify-center flex-shrink-0">
                                        <span class="text-sm font-bold text-white dark:text-black">
                                            {{ substr($ticket->assignedTo->first_name, 0, 1) }}{{ substr($ticket->assignedTo->last_name, 0, 1) }}
                                        </span>
                                    </div>
                                    <div>
                                        <p class="font-medium text-gray-900 dark:text-white text-sm">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ $ticket->created_at->format('M d, Y \a\t g:i A') }}</p>
                            </div>
                        </div>
                        ...
                        @if($ticket->resolved_at)
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ \Carbon\Carbon::parse($ticket->resolved_at)->format('M d, Y \a\t g:i A') }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($ticket->updated_at != $ticket->created_at)
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ $ticket->updated_at->format('M d, Y \a\t g:i A') }}</p>
                                </div>
                            </div>
                        @endif
                    </div>
...
function ticketDetail() {
    return {
        ...
        mentionStartPos: 0,
        allEmployees: {{ Js::from($employeesData) }},
        editForm: {
            title: {{ Js::from($ticket->title) }},
            description: {{ Js::from($ticket->description) }},
            status: {{ Js::from($ticket->status) }},
            priority: {{ Js::from($ticket->priority) }},
            type: {{ Js::from($ticket->type) }},
            assigned_to: {{ Js::from($ticket->assigned_to) }}
        },
        ...
        async loadComments() {
            try {
                const response = await fetch('{{ route("tickets.show", $ticket) }}/comments');
        ...
        async updateTicket() {
            ...
            try {
                const response = await fetch('{{ route("tickets.update", $ticket) }}', {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        async confirmDelete() {
            ...
            try {
                const response = await fetch('{{ route("tickets.destroy", $ticket) }}', {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
