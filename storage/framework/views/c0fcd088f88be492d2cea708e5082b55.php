<div x-data="{
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-gray-600 dark:text-gray-400"><?php echo e($project->tickets->count()); ?> tickets</span>
        </div>

        <button @click="openTicketModal()" class="inline-flex items-center gap-2 px-4 py-2 bg-black dark:bg-white text-white dark:text-black rounded-lg font-semibold hover:opacity-90 transition-all">
    ...
    <?php if($project->tickets->count() > 0): ?>
        <div class="grid grid-cols-1 gap-4">
            <?php $__currentLoopData = $project->tickets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ticket): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div x-show="(filterStatus === 'all' || filterStatus === '<?php echo e($ticket->status); ?>') && (filterPriority === 'all' || filterPriority == <?php echo e($ticket->priority); ?>)"
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="w-12 h-12 rounded-lg bg-gradient-to-br
                            <?php if($ticket->type === 'bug'): ?> from-red-500 to-red-700
                            <?php elseif($ticket->type === 'feature'): ?> from-blue-500 to-blue-700
                            <?php elseif($ticket->type === 'enhancement'): ?> from-purple-500 to-purple-700
                            <?php else: ?> from-gray-500 to-gray-700 <?php endif; ?>
                            flex items-center justify-center text-white flex-shrink-0">
                            <?php if($ticket->type === 'bug'): ?>
                                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M20 8h-2.81c-.45-.78-1.07-1.45-1.82-1.96L17 4.41 15.59 3l-2.17 2.17C12.96 5.06 12.49 5 12 5c-.49 0-.96.06-1.41.17L8.41 3 7 4.41l1.62 1.63C7.88 6.55 7.26 7.22 6.81 8H4v2h2.09c-.05.33-.09.66-.09 1v1H4v2h2v1c0 .34.04.67.09 1H4v2h2.81c1.04 1.79 2.97 3 5.19 3s4.15-1.21 5.19-3H20v-2h-2.09c.05-.33.09-.66.09-1v-1h2v-2h-2v-1c0-.34-.04-.67-.09-1H20V8zm-6 8h-4v-2h4v2zm0-4h-4v-2h4v2z"/>
                                </svg>
                            <?php elseif($ticket->type === 'feature'): ?>
                                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                                </svg>
                            <?php elseif($ticket->type === 'enhancement'): ?>
                                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-start justify-between mb-3">
                                <div>
                                    <div class="flex items-center gap-2 mb-1">
                                        <span class="text-xs font-mono text-gray-500 dark:text-gray-400"><?php echo e($ticket->ticket_number); ?></span>
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold
                                            <?php if($ticket->type === 'bug'): ?> bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400
                                            <?php elseif($ticket->type === 'feature'): ?> bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400
                                            <?php elseif($ticket->type === 'enhancement'): ?> bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400
                                            <?php else: ?> bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 <?php endif; ?>">
                                            <?php echo e(ucfirst($ticket->type)); ?>

                                        </span>
                                    </div>
                                    <h4 class="font-bold text-lg text-gray-900 dark:text-white"><?php echo e($ticket->title); ?></h4>
                                    <?php if($ticket->description): ?>
                                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-2"><?php echo e(Str::limit($ticket->description, 150)); ?></p>
                                    <?php endif; ?>
                                </div>

                                <div class="flex items-center gap-2">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold
                                        <?php if($ticket->status === 'open'): ?> bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400
                                        <?php elseif($ticket->status === 'in-progress'): ?> bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400
                                        <?php elseif($ticket->status === 'resolved'): ?> bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400
                                        <?php else: ?> bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 <?php endif; ?>">
                                        <?php echo e(ucfirst(str_replace('-', ' ', $ticket->status))); ?>
                            ...
                            <div class="flex items-center gap-6 text-sm text-gray-600 dark:text-gray-400">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold
                                        <?php if($ticket->priority === 4): ?> bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400
                                        <?php elseif($ticket->priority === 3): ?> bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400
                                        <?php elseif($ticket->priority === 2): ?> bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400
                                        <?php else: ?> bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 <?php endif; ?>">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <div class="w-6 h-6 rounded-full bg-gradient-to-br from-gray-800 to-black dark:from-gray-700 dark:to-gray-900 flex items-center justify-center text-white text-xs font-bold">
                                                <?php echo e(substr($ticket->assignee->first_name, 0, 1)); ?><?php echo e(substr($ticket->assignee->last_name, 0, 1)); ?>

                                            </div>
                                            <span><?php echo e($ticket->assignee->first_name); ?> <?php echo e($ticket->assignee->last_name); ?></span>
                                        </div>
                                    </div>
                                <?php else: ?>
                                    <div class="flex items-center gap-2">
                            ...
                            <div class="flex items-center gap-2 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                                <?php if($ticket->status !== 'closed'): ?>
                                    <form action="<?php echo e(route('projects.tickets.update', [$project, $ticket])); ?>" method="POST" class="inline">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('PUT'); ?>
                                        <?php if($ticket->status === 'open'): ?>
                                            <input type="hidden" name="status" value="in-progress">
                                            <button type="submit" class="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400 rounded-lg text-sm font-semibold hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors">
                                                Start Progress
                                            </button>
                                        <?php elseif($ticket->status === 'in-progress'): ?>
                                            <input type="hidden" name="status" value="resolved">
                                            <button type="submit" class="inline-flex items-center gap-2 px-3 py-1.5 bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-400 rounded-lg text-sm font-semibold hover:bg-purple-200 dark:hover:bg-purple-900/50 transition-colors">
                                                Mark Resolved
                                            </button>
                                        <?php elseif($ticket->status === 'resolved'): ?>
                                            <input type="hidden" name="status" value="closed">
                                            <button type="submit" class="inline-flex items-center gap-2 px-3 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300 rounded-lg text-sm font-semibold hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                                                Close Ticket
                                            </button>
                                        <?php endif; ?>
                                    </form>
                                ...
                                <button @click="openTicketModal(<?php echo e($ticket); ?>)" class="inline-flex items-center gap-2 px-3 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300 rounded-lg text-sm font-semibold hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <form action="<?php echo e(route('projects.tickets.destroy', [$project, $ticket])); ?>" method="POST" onsubmit="return confirm('Are you sure you want to delete this ticket?')">
                                    <?php echo csrf_field(); ?>
    ...
    <div x-show="showTicketModal"
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full p-6 border border-gray-200 dark:border-gray-700" @click.stop>
                ...
                <form :action="editingTicket ? '<?php echo e(route('projects.tickets.update', [$project, '__TICKET__'])); ?>'.replace('__TICKET__', editingTicket) : '<?php echo e(route('projects.tickets.store', $project)); ?>'" method="POST" class="space-y-4">
                    <?php echo csrf_field(); ?>
...
<?php /**PATH F:\Project\salary\resources\views/projects/tabs/tickets.blade.php ENDPATH**/ ?>