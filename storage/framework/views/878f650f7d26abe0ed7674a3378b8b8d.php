...
<?php $component->withAttributes([]); ?>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm p-6">
                ...
                <div class="space-y-4">
                    ...
                    <!-- Current Labels -->
                    <?php if(!empty($pr['labels']) && count($pr['labels']) > 0): ?>
                        <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                            <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Current Labels</h4>
                            <div class="flex flex-wrap gap-2">
                                <?php $__currentLoopData = $pr['labels']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    window.mergePR = async function(button) {
        ...
        try {
            const response = await fetch('<?php echo e(route('github.pr.merge', $log)); ?>', {
                method: 'POST',
                headers: {
    ...
    window.closePR = async function(button) {
        ...
        try {
            const response = await fetch('<?php echo e(route('github.pr.close', $log)); ?>', {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    'Accept': 'application/json',
    ...
    function prDetailsPage() {
        return {
            ...
            async addLabel() {
                ...
                try {
                    const response = await fetch('<?php echo e(route('github.pr.addLabel', $log)); ?>', {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'Accept': 'application/json',
            ...
            async removeLabel(labelName) {
                ...
                try {
                    const url = '<?php echo e(route('github.pr.removeLabel', ['log' => $log, 'label' => '__LABEL__'])); ?>'.replace('__LABEL__', encodeURIComponent(labelName));
            ...
            async removeReviewer(username) {
                ...
                try {
                    const url = '<?php echo e(route('github.pr.removeReviewer', ['log' => $log, 'username' => '__USERNAME__'])); ?>'.replace('__USERNAME__', encodeURIComponent(username));
            ...
            async removeAssignee(username) {
                ...
                try {
                    const url = '<?php echo e(route('github.pr.removeAssignee', ['log' => $log, 'username' => '__USERNAME__'])); ?>'.replace('__USERNAME__', encodeURIComponent(username));
            ...
            async generateAIReview() {
                ...
                try {
                    const response = await fetch('<?php echo e(route('github.pr.aiReview', $log)); ?>', {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'Accept': 'application/json',
