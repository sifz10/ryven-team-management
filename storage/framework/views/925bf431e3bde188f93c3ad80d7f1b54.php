...
function fileManager() {
    const filesJson = <?php echo json_encode($filesData, 15, 512) ?>;
    ...
    return {
        ...
        async submitFiles() {
            ...
            try {
                const response = await fetch('<?php echo e(route("projects.files.store", $project)); ?>', {
                    method: 'POST',
                    body: formData
                });
        ...
        async confirmDelete() {
            ...
            try {
                const response = await fetch(`/projects/<?php echo e($project->id); ?>/files/${this.fileToDelete.id}`, {
                    method: 'DELETE',
                    headers: {
